...
using (var scope = app.Services.CreateScope())
{
    ...
    // The project uses the existing SQL Server Express instance:
    // .\SQLEXPRESS / LFMS_LocalDb.
    // Do not try to recreate an existing database.
    var connected = false;
    ...
    if (!connected)
    {
        var details = lastError?.GetBaseException().Message ?? "Unknown SQL Server connection error.";

        throw new InvalidOperationException(
            "Cannot connect to SQL Server Express database 'LFMS_LocalDb' on '.\\SQLEXPRESS'. " +
            "Make sure the SQL Server (SQLEXPRESS) service is running and that the LFMS_LocalDb database exists. " +
            "Details: " + details,
            lastError);
    }
...
BEGIN
    ...
    (
        ...
        [Status] nvarchar(100) NOT NULL
            CONSTRAINT [DF_CollectionConfirmations_Status]
            DEFAULT (N'PendingAdminApproval'),
        [ConfirmedAt] datetime2 NOT NULL
            CONSTRAINT [DF_CollectionConfirmations_ConfirmedAt]
            DEFAULT (GETUTCDATE()),
        ...
        CONSTRAINT [PK_CollectionConfirmations]
            PRIMARY KEY ([Id]),

        CONSTRAINT [FK_CollectionConfirmations_Posts]
            FOREIGN KEY ([PostId])
            REFERENCES [Posts]([Id])
            ON DELETE NO ACTION,

        CONSTRAINT [FK_CollectionConfirmations_AspNetUsers]
            FOREIGN KEY ([UserId])
            REFERENCES [AspNetUsers]([Id])
            ON DELETE NO ACTION
    );
...
BEGIN
    UPDATE [CollectionConfirmations]
    SET [Status] = N'PendingAdminApproval'
...
IF OBJECT_ID(N'[CollectionConfirmations]', N'U') IS NOT NULL
BEGIN
    IF COL_LENGTH(N'CollectionConfirmations', N'ClaimantVerificationAnswer') IS NULL
        ALTER TABLE [CollectionConfirmations]
        ADD [ClaimantVerificationAnswer] nvarchar(2000) NULL;

    IF COL_LENGTH(N'CollectionConfirmations', N'VerificationReferenceAtSubmission') IS NULL
        ALTER TABLE [CollectionConfirmations]
        ADD [VerificationReferenceAtSubmission] nvarchar(2000) NULL;

    IF COL_LENGTH(N'CollectionConfirmations', N'OwnerApprovalUserId') IS NULL
        ALTER TABLE [CollectionConfirmations]
        ADD [OwnerApprovalUserId] nvarchar(450) NULL;

    IF COL_LENGTH(N'CollectionConfirmations', N'OwnerApprovedAt') IS NULL
        ALTER TABLE [CollectionConfirmations]
        ADD [OwnerApprovedAt] datetime2 NULL;

    IF COL_LENGTH(N'CollectionConfirmations', N'AdminApprovalUserId') IS NULL
        ALTER TABLE [CollectionConfirmations]
        ADD [AdminApprovalUserId] nvarchar(450) NULL;

    IF COL_LENGTH(N'CollectionConfirmations', N'AdminApprovedAt') IS NULL
        ALTER TABLE [CollectionConfirmations]
        ADD [AdminApprovedAt] datetime2 NULL;

    IF COL_LENGTH(N'CollectionConfirmations', N'ReviewNotes') IS NULL
        ALTER TABLE [CollectionConfirmations]
